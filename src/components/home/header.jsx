...
import HeaderDropdown from "@/components/home/header-dropdown";
import { useState } from "react";
import { ChevronDown, ChevronRight } from "lucide-react";
import SignInSidebar from "@/components/home/signin-sidebar";
...
export default function Header() {
  ...
  return (
    <>
      <header className="absolute top-0 z-10 w-full px-4 py-4 md:px-12">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button className="cursor-pointer bg-violet-600 hover:bg-violet-700 text-white font-medium px-4 py-1 text-sm rounded" onClick={() => setActiveSidebar(true)}>
              Sign In
            </Button>
          </div>
        </div>
      ...
      <SignInSidebar isOpen={activeSidebar} onClose={() => setActiveSidebar(false)} />
    </>
  );
}

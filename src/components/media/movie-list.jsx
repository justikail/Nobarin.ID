...
export default function MovieList({ data, genres }) {
  const tmdbImageUrl = process.env.NEXT_PUBLIC_TMDB_IMAGE_URL;

  return (
    <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
      {data.map((item, index) => (
        <Link key={index} href={formattedTitle("movie", item.id, item.title)} className="group">
          <div className="overflow-hidden rounded-md">
            <div className="aspect-[2/3] relative">
              <Image src={item.poster_path ? `${tmdbImageUrl}${item.poster_path}` : "/asset/svg/placeholder.svg"} alt={`Poster ${item.title}`} fill className="object-cover transition-transform duration-300 group-hover:scale-110" />
          ...
          <p className="text-xs text-gray-400">
            {item.release_date ? new Date(item.release_date).getFullYear() : "-"} • {getGenreNames(item.genre_ids, genres)}
          </p>
        </Link>
      ))}
    </div>
